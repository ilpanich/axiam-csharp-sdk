...
| C# | implemented — `AxiamMcp.ProtectedResourceMetadata`/`AxiamMcp.BearerChallenge` (pure, `Axiam.Sdk`) plus the ASP.NET Core middleware option and minimal-API endpoint (`Axiam.Sdk.AspNetCore`). Rule 8's optional gRPC `WWW-Authenticate` metadata attachment does not apply: this SDK's `Grpc`/`Amqp` namespaces are its own client transports to AXIAM (an authz-check client, an HMAC-verifying AMQP consumer), not a guard authenticating inbound calls from an MCP client, so there is no gRPC/AMQP guard here for the rule to extend |
| Rust, Python, Java, Kotlin, PHP, Go, Swift, C, C++ | not yet |
