...
- **Re-vendor CONTRACT.md/openapi.json/management-registry.json/proto/ to
  ...
  both from `mfa-first-login-and-csr-issuance-plan.md`'s F-1 fan-out.

- **`ManagementApi.Certificates.SignCsrAsync`** — issue an end-entity
  certificate for a key AXIAM never sees. `POST /api/v1/certificates/sign-csr`
  takes a caller-built PKCS#10 CSR (a new `SignCertificateCsrRequest`, generated
  from the vendored spec) and returns the existing `Certificate` — **not**
  `GeneratedCertificate` — because there is no private key to return and no
  field to leave empty. The subscriber's key is produced by whoever built the
  CSR — an offline ceremony, an HSM, a device's secure element — and crosses
  the wire in neither direction; `certificates:generate` is the only
  permission it needs, the same precedent `signing-cas/sign-csr` already set.
  Generated by `scripts/gen_management.py`; the §27.9 surface count moves
  159 → 160. A new semantics test asserts, reflectively, that `Certificate`
  carries no private-key property at all, and round-trips a response through
  `SignCsrAsync` to prove the decode needs none.

- **`AxiamClient.WebauthnSetupRegisterStartAsync` /
  `WebauthnSetupRegisterFinishAsync`** (CONTRACT.md §24.1, §25.1–§25.2,
  contract 1.45) — enrol a passkey or a security key, instead of TOTP, as the
  first factor during forced MFA setup at login. The WebAuthn twins of
  `MfaSetupEnrollAsync`/`MfaSetupConfirmAsync`: a tenant that requires MFA no
  longer forces a new user to own a TOTP app to get in.

  Both calls take the login's setup token as their **only** credential and,
  per §24.1, run over a transport dedicated to exactly that: a second,
  permanently cookie-free `HttpClient` this client now owns alongside its
  normal one, so neither an `Authorization` header nor a session cookie ever
  reaches the wire on these two calls even when this client instance already
  has an active session elsewhere. A successful `WebauthnSetupRegisterFinishAsync`
  still adopts the new session it creates — exactly as `MfaSetupConfirmAsync`
  does — by copying the Set-Cookie triple the anonymous transport captured
  into this client's own cookie jar (the new internal `CookieJarBridge`, unit
  tested on its own) and feeding the response through the same
  `X-CSRF-Token` capture every other call already goes through.

  `setup_token` and `state_token` are both `Sensitive<T>`, matching this SDK's
  existing convention for both fields (§25.3, §24.5). A `403` from `finish`
...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  1.44).** The new `RevocationFeed`, passed as the `revocationFeed` argument to
  `JwksVerifier`, plus `RevocationFeed.EntryFor` and the three bounds the
  contract names (`MinPollInterval`, `DefaultPollInterval`, `MaxEntries`).
  ...
  `null`, so every existing call site compiles and behaves exactly as before. It
  never fetches on the request path after the first call — `VerifyAsync` reads a
  ...
  no fallback to `jti`. A `RevocationFeed` is safe for concurrent use, so one
  instance can serve several verifiers.
...
- **A malformed `mtls_endpoint_aliases` entry now throws instead of falling
  ...
  leaves every other endpoint working, and a client configured without
  `ClientCertificatePem` never reads the member at all.
...
- **RFC 8705 §5 `mtls_endpoint_aliases` (SDK contract 1.40, CONTRACT.md §21.3
  ...
  property (the new `MtlsEndpointAliases` record), and the §12 helpers now
  prefer an alias over the top-level entry of the same name on any call made
  over mutual TLS — that is, from a client carrying `ClientCertificatePem`.
  Six operations reach the token endpoint (`OidcExchangeAsync`,
  `OidcRefreshAsync`, `LoginClientCredentialsAsync`, `DevicePollAsync`,
  `TokenExchangeAsync`, `UmaExchangeTicketAsync`), plus `IntrospectAsync`,
  `RevokeAsync`, `DeviceAuthorizeAsync` and `OidcParAsync`.
  ...
  The new record component is optional with a default, so existing positional
...
- **§5.2.2 rule 4 is new, and is an errata rather than a wire change.** The
  ...
  `X-Tenant-ID` is set in one place, `Rest/AxiamHttpMessageHandler.cs`, where
  the `Remove` is the normalising half of an unconditional remove-then-set
  rather than an endpoint exemption.
...
- **§27: service accounts as RBAC principals** (contract 1.34) — eight generated
  ...
  need. `RolesApi.UnassignFromServiceAccountAsync` takes the same optional
...
- **§5.2.2: the acting tenant and the principal tenant are different things**
  (contract 1.34). `LoginResult` gains a `Scope` component — a new
  `PrincipalScope` record carrying `ActingTenantId`, `PrincipalTenantId`,
  `PrincipalTenantSlug`, `OrgId` and (from §5.2.3) `ReachableTenantIds`.

  Grouped into one record rather than five more parameters on `LoginResult`
  because they are read together and grow together; a record that gains a
  parameter per contract revision is one whose shape churns every time.

  `PrincipalTenantId` falls back to `ActingTenantId` when the server omits it.
...
- **A registration record for your own password was sealed against the wrong
  ...
  `OpaqueEnrollmentAsync` had one behaviour for a method documented for three
  ...
  `OpaqueEnrollmentForSelfAsync` seals against the principal tenant captured at
  login (dropping the `tenant_slug` that would otherwise out-vote the id
...
- **`tenant_scope: []` no longer reaches the wire** (§5.2.3 rule 1, refused with
  `400`). `JsonIgnoreCondition.WhenWritingNull` drops a null but not an empty
  collection — and an empty collection is exactly what building the field from a
  filtered sequence produces for "no tenants named". `scripts/gen_management.py`
  now emits a normalising `init` accessor for `TenantScope` that stores an empty
  list as `null`.
...
Documentation added here names `X-Axiam-Tenant`, because a tenant switch sent under
the other name is not refused — it is ignored, and the request quietly acts on the
principal's own tenant instead. The discrepancy has been reported upstream; this
...
- Attest the published packages, pin setup-python by digest, re-vendor contract 1.33
...
- **CONTRACT 1.32 — signing in an organization-level principal (§5.2.1).**
  ...
  ```csharp
  new AxiamClient(baseUrl, "organization", new AxiamClientOptions { OrgSlug = "globex" })
  ```
...
- **`TenantContext` now rejects a blank `orgSlug`** (CONTRACT.md §5.1, §5.2.1
  rule 2). `tenantId` was already checked with `IsNullOrWhiteSpace`; `orgSlug`
  accepted `""` and put it on the wire. A **null** `orgSlug` stays accepted —
  that is the organization identifier being optional, not blank.
...
- **The §27 namespace handles now sit directly on the client** — `client.Roles`,
  ...
  Each direct property forwards to `Management`, so rule 4's "where an SDK offers both, the
  two MUST return equivalent handles" holds structurally rather than by two code paths
  agreeing to stay in step. `ManagementClientAccessorsTests` asserts it by comparing the
