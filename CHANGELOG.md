...
- **§5.2.2 rule 4 is new, and is an errata rather than a wire change.** The
  ...
  `X-Tenant-ID` is set in one place, `Rest/AxiamHttpMessageHandler.cs` — the
  `Remove` there is the normalising half of an unconditional remove-then-set,
  not an endpoint exemption.
